...
We use _toggle buttons_ in simulation design to communicate to learners they can toggle between two defined states. A toggle button interaction is similar to a checkbox in that it toggles between two states. Unlike a checkbox, however, a _toggle button_ does not have strictly defined 'checked' and 'unchecked' states for visual presentation and for accessibility. _Toggle button_ design is much more flexibile. This flexibility allows designers to make these interactions fun and inviting to help communicate their importance within the simulation experience.  

PhET has many options for _toggle buttons_. They can be big or small. They can be 'round' or 'rectangular'. They can be 'momentary' or 'sticky.' They can have a look that is pressed or unpressed. They can have a visual name or an icon that changes to display one of two states. They can be placed directly on a component to look like an on-off button.

For accessibility, _toggle buttons_ can be "configured" (i.e. coded) so they communicate the interaction as intended when a learner is using assitive technologies.
...
* **Warning:** When a toggle button turns a component on, and that action starts a series of events that are central to the simulation's learning goals, it may not be possibe to make the interaction fully accessible in the _Core Description_ design phase (e.g., _Molecules and Light_, _Models of the Hydrogen Atom_).

#### Description Design Needs for Toggle Buttons:
Generally, a toggle button requires an accessible name, it may benefit from accessible help text, and it requires two accessible context responses to confirm the result of the changed state as the button is toggled during interaction. Designers need to make sure the description design fits the chosen **accessibleRoleConfiguration** for accessibility.

##### Accessible Role Configurations
* **'toggle' option** - typically works well with a single unique accessible name, and 2 context responses that confirm the changed states when toggled. The pressed state is communicated through the code to screen reader users. The button may also be communicated as a "toggle button" rather than just "button."
...
* **'button' option (text)** - when two visually displayed names represent the boolean states, it works best to have matching unique accessible names, and 2 context responses that confirm the changed states.  
* **'button' option (icons)** - when two visually displayed icons represent the boolean states, it can work well with dynamic names that match the icons and 2 confirmatory context responses. It may also work well to use the 'toggle' configiration with a single unique name. The configuration depends on the choice of the accessible names or name. Since the words are not visually displayed, there is some wiggle room in the accessible design. The accessible design of iconic 'boolean' toggle buttons can be tricky for description and voicing features. Some commonly used iconic buttons are already handled in common code (e.g., Pause/Play Button, Eye Toggle Button, Timer Button).

  **Note:** Design pattern for the A-B Switch is written up seperately, but I might add it here. It works like the button option.

...
type SelfOptions = {
   ...
   */
  createContextResponseAlert?: CreateTextFunction<number> | null;
  ...
  // alertDelay for the context response to. This value should be small enough that it feels like you are
...
const AccessibleValueHandler = <SuperType extends Constructor<Node>>( Type: SuperType, optionsArgPosition: number ): SuperType & Constructor<TAccessibleValueHandler> => {
  const AccessibleValueHandlerClass = DelayedMutate( 'AccessibleValueHandler', ACCESSIBLE_VALUE_HANDLER_OPTIONS,
    class AccessibleValueHandler extends Voicing( Type ) implements TAccessibleValueHandler {
      ...
      private _createContextResponseAlert: CreateTextFunction<number> | null = null;

      // The Property value when an interaction starts, so it can be used as the "old" value
      // when generating a context response at the end of an interaction with createContextResponseAlert.
      private _valueOnStart: number;

      // The utterance sent to the utteranceQueue when the value changes, alert content generated by
      // optional createContextResponseAlert. The alertDelay on this utterance will increase if the input
      ...
      // if using createContextResponseAlert
      ...
      public set createContextResponseAlert( value: CreateTextFunction<number> | null ) {
        this._createContextResponseAlert = value;
      }

      public get createContextResponseAlert(): CreateTextFunction<number> | null {
      ...
      public alertContextResponse(): void {
        ...
        if ( this._createContextResponseAlert ) {
          ...
          const mappedValue = this._getMappedValue();
          const endInteractionAlert = this._createContextResponseAlert( mappedValue, this._valueProperty.value, this._valueOnStart );
          ...
          if ( endInteractionAlert ) {
            ...
            this.forEachUtteranceQueue( ( utteranceQueue: UtteranceQueue ) => {
              ...
              this._contextResponseUtterance.alertDelay = Math.min( this._contextResponseMaxDelay,
